...
// Note: if the buf contains an exception, you should call UnmarshalThriftException instead.
func UnmarshalThriftData(ctx context.Context, codec remote.PayloadCodec, buf []byte, data interface{}) error {
	c, ok := codec.(*thriftCodec)
	if !ok {
		c = defaultCodec
	}
	tProt := NewBinaryProtocol(remote.NewReaderBuffer(buf))
	err := c.unmarshalThriftData(ctx, tProt, data, len(buf))
	if err != nil {
		tProt.Recycle()
	}
	return err
...
// unmarshalThriftData only decodes the data (after methodName, msgType and seqId)
func (c thriftCodec) unmarshalThriftData(ctx context.Context, tProt *BinaryProtocol, data interface{}, dataLen int) error {
...
func decodeBasicThriftData(ctx context.Context, tProt thrift.TProtocol, data interface{}) error {
	var err error
	switch t := data.(type) {
	case MessageReader:
	...
	case MessageReaderWithMethodWithContext:
		// methodName is not used in thrift data
		if err = t.Read(ctx, "", tProt); err != nil {
			return remote.NewTransError(remote.ProtocolError, err)
		}
	default:
