...
const (
	...
	IntKeyFrameType  = 31 // TODO: move to `transmeta`
	FrameTypeMeta    = "1"
	FrameTypeHeader  = "2"
	FrameTypeData    = "3"
	FrameTypeTrailer = "4"

	StrKeyMetaData = "metadata"
)
...
func (h *Header) SetSeqNum(seqNum uint32) {
	h.seqNum = seqNum
}
...
func (h *Header) WriteWithSize(buf []byte, bufSize int) error {
	...
	binary.BigEndian.PutUint32(buf[OffsetSeqNum:OffsetSeqNum+4], h.seqNum)
...
func (h *Header) Read(input []byte) (err error) {
	...
	h.seqNum = binary.BigEndian.Uint32(buf[OffsetSeqNum : OffsetSeqNum+4])
