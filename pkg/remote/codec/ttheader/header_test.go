...
func TestHeader_SeqNum(t *testing.T) {
	h := NewHeader()
	test.Assert(t, h.SeqNum() == 0)
	h.SetSeqNum(10)
	test.Assert(t, h.SeqNum() == 10)
}
...
func TestHeader_WriteWithSize(t *testing.T) {
	t.Run("fixed", func(t *testing.T) {
		...
		test.Assert(t, binary.BigEndian.Uint32(buf[4:8]) == h.seqNum, buf[4:8])
...
func TestHeader_Read(t *testing.T) {
	...
	t.Run("fixed+token", func(t *testing.T) {
		...
		test.Assert(t, hr.seqNum == hw.seqNum, hr.seqNum)
