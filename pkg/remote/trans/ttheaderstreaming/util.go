...
func getSeqID(m remote.Message) int32 {
	if seqID, ok := m.Tags()[ttheader.TagReceivedSeqID]; !ok {
		return 0 // invalid seq id, won't be generated by Kitex
	} else if v, ok := seqID.(int32); !ok {
		return 0
	} else {
		return v
	}
...
func getFrameType(m remote.Message) string {
	if frameType, exists := m.TransInfo().TransIntInfo()[ttheader.IntKeyFrameType]; exists {
		return frameType
	}
