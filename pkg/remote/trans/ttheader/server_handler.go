...
const (
	...
	FlagStreamingBitMask = 0b1000_0000_0000_0000
...
func (t *ttheaderStreamingTransHandler) ProtocolMatch(ctx context.Context, conn net.Conn) (err error) {
	...
	if magic == HeaderMagic {
		...
		if flags&FlagStreamingBitMask != 0 {
			return nil
		}
	}
...
func (t *ttheaderStreamingTransHandler) OnRead(ctx context.Context, conn net.Conn) error {
	ri, rCtx, msg, err := t.readHeaderFrame(ctx, conn)
	if err != nil {
	...
	for _, handler := range t.opt.StreamingMetaHandlers {
		if rCtx, err = handler.OnReadStream(rCtx); err != nil {
			f := newEndOfStreamFrame(ctx, CodeInternal, err.Error(), nil)
			if sendErr := writeFrame(ctx, conn, f); sendErr != nil {
	...
	defer func() {
		...
		if panicErr != nil {
			if conn != nil {
				klog.CtxErrorf(rCtx, "KITEX: TTHeader Streaming OnRead panic happened, close conn, remoteAddress=%s, error=%s\nstack=%s",
					conn.RemoteAddr(), panicErr, string(debug.Stack()))
			} else {
				klog.CtxErrorf(rCtx, "KITEX: TTHeader Streaming OnRead panic happened, error=%v\nstack=%s",
	...
	st := newTTHeaderStreamWithMW(rawStream, t.opt.RecvEndpoint, t.opt.SendEndpoint)
	...
	if methodInfo == nil {
		err = t.unknownMethod(rCtx, ri, st, svcInfo)
	} else {
		// TODO: support ServerStreamingGetRequest
		err = t.invokeHandler(rCtx, &streaming.Args{Stream: st}, nil)
	}

	if err = t.finishStream(rCtx, conn, ri, st, err); err != nil {
