...
var (
	_ remote.TransReadWriter = (*cliTransHandler)(nil)
	_ remote.StreamAllocator = (*cliTransHandler)(nil)
)
...
func (c *cliTransHandler) NewStream(
	...
	st := newTTHeaderStream(ctx, conn, msg, c.codec)
	...
	if err := writeFrame(ctx, conn, headerFrame); err != nil {
		return nil, err
	}

	// TODO: reading meta frame here may cause much larger latency than GRPC.
	// Maybe it's better a lazy-read? Or it's not enabled by default unless Kitex detects mesh is enabled.

	// Streaming over TTHeader requires the server returns a meta frame before server-header-frame
	metaFrame, err := readFrame(conn)
	if err != nil {
		return nil, err
	}
	if metaFrameHandler != nil {
		if ctx, err = metaFrameHandler(ctx, msg, metaFrame); err != nil {
			return nil, err
		}
	}
	return st, nil
